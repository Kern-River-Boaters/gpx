...
- **Multi-Persona Venue Contracts**:
  ...
  - **Patent Counsel Persona**: Hardware-agnostic functional abstractions, Claims 1/15/25 outlines, multi-embodiment architectures, prior art matrix.
...
## 3. ECC Learnings for Nightly Coding Workers
- **Never Guess Contracts**: Read declarative skill files before implementing script logic.
- **Mandatory 2-Pass Validation**: Always run a critique pass (`generate` -> `critique` -> `refine`).
