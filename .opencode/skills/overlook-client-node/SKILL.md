...
4. **Identity & Bi-Directional SSH Mesh:** The primary OS user on Overlook is standardized as `jpino` (`/home/jpino`), aligning 1:1 with the Kern compute brain (`jpino@kern`) and Moab workstation (`jpino@moab`). Overlook runs `openssh-server` (`systemctl enable --now ssh`, UFW allow 22) with Kern's public key (`jpino@keysight.com`) in `~/.ssh/authorized_keys` to enable inbound SSH from Kern. Outbound SSH configurations to Kern map `User jpino` in `~/.ssh/config`.
