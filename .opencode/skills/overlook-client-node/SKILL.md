...
description: "overlook-client-node skill for OpenCode"
---
...
1. **Prevent Dual-GPU Sleep Lockup:** The Haswell CPU + AMD Radeon R9 M370X dGPU suffers from unrecoverable PCIe VM context loss (`amdgpu_vm_validate failed`) when entering S3 sleep on lid close. `HandleLidSwitch=ignore` MUST be enforced in `/etc/systemd/logind.conf.d/lid-switch.conf` and GNOME power schemas (`lid-close-*-action='nothing'`).
