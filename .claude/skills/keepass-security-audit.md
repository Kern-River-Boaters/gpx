...
## 🗂️ 4. Target KeePass Group Restructuring Hierarchy & Custom Emoji Icons

The canonical, organized KeePass database hierarchy for **Jose Database.kdbx** consists of **21 top-level groups** and **7 archive subgroups**, each with an embedded 48×48 RGBA custom icon rendered from `Noto Color Emoji`:

```
Jose Database.kdbx
├── ☁️ Identity & Cloud/            (Google, Apple ID, Microsoft, Password Manager, AWS, Obsidian, Bitwarden)
...
├── 🏛️ Government/                  (Social Security, IRS, CA MyFTB, DMV, Passports, USCIS, IHSS, EDD, TCRC)
...
├── 🚵 Outdoors & Sports/           (Kern River Boaters, Kayaking, Strava, Garmin inReach, State Parks, CRPD, Ride On, AYSO)
├── 🛒 Shopping/                    (Amazon, Costco, Target, Best Buy, Sweet Maria's Coffee, Care.com)
├── ✈️ Travel/                      (Airlines [Delta, Alaska, AA, Southwest], Hotels, Rental Cars, Amtrak)
├── ⚡ Utilities/                   (SoCalGas, SCE, Water, Trash, Verizon, Fastrak, Frontier, UPS, FedEx)
├── 🖥️ Smart Home & Network/        (Home router, Synology NAS, Wi-Fi networks, IoT devices, TP-Link, ANOVA, Roast Pilot, Grundfos)
├── 📜 Software Licenses/           (Active software licenses, serial numbers, product keys)
├── 🔐 KeePass & Encryption Keys/   (GPG keys, recovery codes, database master keys, backup codes)
...
└── 📦 Archive/
    ...
    ├── 🗳️ Civic - CVUSD Campaign/ (Jose past school district & board campaign accounts, ActBlue)
    ├── 🤝 Civic - LULAC & Buen Vecino/ (Jose past LULAC & Buen Vecino community leadership)
    ├── 🗄️ Past Work - Lisa/       (CollegeAdvisor, Defunct La Reina - lpino@lareina.com)
    ├── 🗄️ Past Work - Jose/       (Jose Agilent pre-spinoff accounts - jpino@agilent.com)
    ├── 💾 Legacy Software/        (Discontinued OS tools, PalmOS, legacy utilities)
    └── 🚫 Defunct Services/       (eBags, Circuit City, CompUSA, Borders, DVD X Copy, Adelphia, Old ISPs)
```

### 🎨 Custom Emoji Icon Engine Standard
1. **Header Embedding**: 48×48 RGBA PNGs generated from `Noto Color Emoji` are embedded in `<Meta><CustomIcons><Icon>`.
2. **KeePassXC Priority**: To ensure custom icons display in the KeePassXC tree sidebar, every `<Group>` element MUST have `<IconID>0</IconID>` paired directly with `<CustomIconUUID>b64_uuid</CustomIconUUID>`.
3. **Stale Reference Safeguard**: When updating custom icons, all existing `<CustomIcons>` nodes in `<Meta>` and `<CustomIconUUID>` tags on groups must be cleanly purged and re-linked to prevent fallback to the default key icon.

---

## 🛡️ 5. Security Triage & Remediation Strategy

1. **High-Risk Account Prioritization**:
   * Critical risk categories: **`Finance`**, **`Identity & Cloud`**, **`Government`**, **`Healthcare`**, **`Work - Keysight`**, and **`Smart Home & Network`**.
...
### 1. Interactive Vault Reorganization
```bash
# Reorganize primary vault with clean custom emoji icons
python3 Common/_Meta/Scripts/keepass_reorganize_vault.py

# Generate icon test sandbox database for GUI verification
python3 Common/_Meta/Scripts/create_icon_preview_vault.py
```

### 2. High-Risk Account Audit & Prioritization
```bash
# Export and audit high-risk accounts sorted by newest password age
...
```bash
# Launch browser assist for high-risk missing-2FA accounts (skips adult kids automatically)
